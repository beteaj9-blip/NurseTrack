      ...
      <main className="p-[clamp(24px,4vw,42px)] content-start grid gap-4 w-full pt-8">
        <section className="mt-6 grid grid-cols-2 gap-6 items-stretch max-[980px]:grid-cols-1">
          ...
          <article className="mt-0 p-[1.45rem] rounded-lg border border-[#e2e8f0] bg-white shadow-[0_16px_44px_rgba(32,33,36,0.07)] flex flex-col">
            ...
            <form className="grid grid-cols-2 gap-4 gap-y-5 items-start flex-1 max-[780px]:grid-cols-1" onSubmit={handleAddDutyArea}>
              <label className="flex flex-col gap-1.5 m-0 !text-sm !font-bold !text-[#344054]" htmlFor="duty-area-name">Duty Area<input className={inputClass} id="duty-area-name" name="dutyAreaName" type="text" placeholder="Emergency Room" required /></label>
              <label className="flex flex-col gap-1.5 m-0 !text-sm !font-bold !text-[#344054]" htmlFor="duty-area-hospital">Hospital<InlineSelect value={newDutyHospitalId} options={hospitalOptions} placeholder="Select hospital" onChange={setNewDutyHospitalId} /></label>
              <div className="col-span-full grid grid-cols-[minmax(0,1fr)_auto] gap-4 items-center mt-[0.15rem] max-[780px]:grid-cols-1"><div className="w-full min-h-[56px] flex items-center m-0 p-[0.9rem_1rem] bg-[#f8fafc] border border-[#e2e8f0] rounded-lg !text-[#4c5d7d] !text-sm !font-bold" role="status">Save appends this ward to the hospital record.</div><div className="flex justify-end items-center max-[780px]:justify-stretch"><button className="inline-flex items-center justify-center w-auto min-w-[210px] min-h-[50px] px-[30px] whitespace-nowrap rounded-[10px] bg-[#8A252C] !text-white !text-[0.95rem] !font-extrabold shadow-[0_10px_22px_rgba(138,37,44,0.18)] hover:bg-[#6d1d23] hover:shadow-[0_16px_34px_rgba(138,37,44,0.22)] transition-all cursor-pointer disabled:opacity-60 max-[780px]:w-full max-[780px]:min-w-0" type="submit" disabled={addHospitalWard.isPending || !hospitals.length}>Add Duty Area</button></div></div>
            </form>
          </article>
        </section>

        <section className="mt-6 p-[1.45rem] rounded-lg border border-[#e2e8f0] bg-white shadow-[0_16px_44px_rgba(32,33,36,0.07)]"><div className="flex items-center justify-between gap-4 mb-4 pb-4 border-b border-[#e5eaf1] flex-wrap"><div><h2 className="m-0 !text-[#111827] !text-[1.25rem] leading-[1.15] !font-bold">Hospital List</h2></div><span className="inline-flex items-center px-[10px] py-[4px] rounded-full !text-[0.76rem] !font-extrabold whitespace-nowrap bg-[#e9f8ef] !text-[#078033]">{isLoading ? "Loading" : `${activeHospitals.length} active`}</span></div>
          <div className="grid grid-cols-[minmax(0,1fr)_190px] gap-3 mb-4 max-[780px]:grid-cols-1"><label className="flex flex-col gap-1.5 m-0 !text-sm !font-bold !text-[#344054]" htmlFor="hospital-list-search">Search Hospitals<input className={inputClass} id="hospital-list-search" type="search" placeholder="Search hospital name or code" value={hospitalSearch} onChange={(event) => { setHospitalSearch(event.target.value); setHospitalPage(1); }} /></label><label className="flex flex-col gap-1.5 m-0 !text-sm !font-bold !text-[#344054]">Status Filter<InlineSelect value={hospitalStatus} options={statusOptions} placeholder="All" onChange={(value) => { setHospitalStatus(value as StatusFilter); setHospitalPage(1); }} /></label></div>
          <div className="w-full overflow-hidden border border-[#dbe3ee] rounded-xl max-[780px]:overflow-x-auto max-[780px]:overflow-y-hidden"><div className="grid grid-cols-[minmax(260px,1fr)_minmax(120px,180px)_minmax(130px,170px)_110px] items-center gap-4 w-full p-[1rem_1.25rem] bg-[#f8fafc] border-b border-[#dbe3ee] max-[780px]:min-w-[760px]"><span className="min-w-0 whitespace-nowrap !text-[#0b1b3a] !text-[0.85rem] !font-extrabold uppercase">Hospital</span><span className="min-w-0 whitespace-nowrap !text-[#0b1b3a] !text-[0.85rem] !font-extrabold uppercase">Code</span><span className="min-w-0 whitespace-nowrap !text-[#0b1b3a] !text-[0.85rem] !font-extrabold uppercase">Status</span><span className="justify-self-end whitespace-nowrap !text-[#0b1b3a] !text-[0.85rem] !font-extrabold uppercase">Action</span></div>
            {isLoading ? <div className="flex items-center justify-center gap-3 p-6 text-center !text-[#64748b] !text-sm !font-bold"><span className="h-4 w-4 animate-spin rounded-full border-2 border-[#e2e8f0] border-t-[#8A252C]" aria-hidden="true" />Loading hospitals...</div> : pagedHospitals.map(hospital => <div className="grid grid-cols-[minmax(260px,1fr)_minmax(120px,180px)_minmax(130px,170px)_110px] items-center gap-4 w-full p-[1rem_1.25rem] border-b border-[#e5eaf1] last:border-b-0 max-[780px]:min-w-[760px]" key={hospital.id}><span className="min-w-0 whitespace-nowrap !text-[#111827] !text-sm"><strong className="!font-[800] whitespace-nowrap">{hospital.fullName || hospital.name}</strong></span><span className="min-w-0 whitespace-nowrap !text-[#4c5d7d] !text-sm !font-semibold">{hospital.name}</span><span className={`w-fit rounded-full px-3 py-1 !text-[0.76rem] !font-extrabold ${statusBadge(isActive(hospital.active))}`}>{isActive(hospital.active) ? "Active" : "Deactivated"}</span><span className="justify-self-end whitespace-nowrap"><button className="flex items-center justify-center w-[88px] min-w-[88px] min-h-[42px] px-[18px] whitespace-nowrap rounded-lg bg-white border border-[#e2e8f0] !text-[#334155] !text-sm !font-bold hover:bg-[#f8fafc] transition-colors cursor-pointer" type="button" onClick={() => setEditingHospital(hospital)}>Edit</button></span></div>)}
          ...
          {hospitalTotalPages > 1 && <div className="flex justify-between items-center p-[1rem_1.5rem] border border-[#e2e8f0] border-t-0 rounded-b-lg bg-[#f8fafc]"><button className={ghostBtn} onClick={() => setHospitalPage(page => Math.max(1, page - 1))} disabled={hospitalPage === 1}>Previous</button><span className="!text-[0.875rem] !font-[600] !text-[#64748b]">Page {hospitalPage} of {hospitalTotalPages}</span><button className={ghostBtn} onClick={() => setHospitalPage(page => Math.min(hospitalTotalPages, page + 1))} disabled={hospitalPage === hospitalTotalPages}>Next</button></div>}
        </section>

        <section className="mt-6 p-[1.45rem] rounded-lg border border-[#e2e8f0] bg-white shadow-[0_16px_44px_rgba(32,33,36,0.07)]"><div className="flex items-center justify-between gap-4 mb-4 pb-4 border-b border-[#e5eaf1] flex-wrap"><div><h2 className="m-0 !text-[#111827] !text-[1.25rem] leading-[1.15] !font-bold">Duty Area List</h2></div><span className="inline-flex items-center px-[10px] py-[4px] rounded-full !text-[0.76rem] !font-extrabold whitespace-nowrap bg-[#e9f8ef] !text-[#078033]">{isLoading ? "Loading" : `${dutyAreas.filter(area => area.active).length} active`}</span></div>
          <div className="grid grid-cols-[minmax(0,1fr)_190px] gap-3 mb-4 max-[780px]:grid-cols-1"><label className="flex flex-col gap-1.5 m-0 !text-sm !font-bold !text-[#344054]" htmlFor="duty-area-list-search">Search Duty Areas<input className={inputClass} id="duty-area-list-search" type="search" placeholder="Search duty area or hospital" value={dutyAreaSearch} onChange={(event) => { setDutyAreaSearch(event.target.value); setDutyAreaPage(1); }} /></label><label className="flex flex-col gap-1.5 m-0 !text-sm !font-bold !text-[#344054]">Status Filter<InlineSelect value={dutyAreaStatus} options={statusOptions} placeholder="All" onChange={(value) => { setDutyAreaStatus(value as StatusFilter); setDutyAreaPage(1); }} /></label></div>
          <div className="w-full overflow-hidden border border-[#dbe3ee] rounded-xl max-[780px]:overflow-x-auto max-[780px]:overflow-y-hidden"><div className="grid grid-cols-[minmax(260px,1fr)_minmax(120px,180px)_minmax(130px,170px)_110px] items-center gap-4 w-full p-[1rem_1.25rem] bg-[#f8fafc] border-b border-[#dbe3ee] max-[780px]:min-w-[760px]"><span className="min-w-0 whitespace-nowrap !text-[#0b1b3a] !text-[0.85rem] !font-extrabold uppercase">Duty Area</span><span className="min-w-0 whitespace-nowrap !text-[#0b1b3a] !text-[0.85rem] !font-extrabold uppercase">Hospital</span><span className="min-w-0 whitespace-nowrap !text-[#0b1b3a] !text-[0.85rem] !font-extrabold uppercase">Status</span><span className="justify-self-end whitespace-nowrap !text-[#0b1b3a] !text-[0.85rem] !font-extrabold uppercase">Action</span></div>
            {isLoading ? <div className="flex items-center justify-center gap-3 p-6 text-center !text-[#64748b] !text-sm !font-bold"><span className="h-4 w-4 animate-spin rounded-full border-2 border-[#e2e8f0] border-t-[#8A252C]" aria-hidden="true" />Loading duty areas...</div> : pagedDutyAreas.map(area => <div className="grid grid-cols-[minmax(260px,1fr)_minmax(120px,180px)_minmax(130px,170px)_110px] items-center gap-4 w-full p-[1rem_1.25rem] border-b border-[#e5eaf1] last:border-b-0 max-[780px]:min-w-[760px]" key={`${area.hospitalId}-${area.name}-${area.active ? "active" : "inactive"}`}><span className="min-w-0 whitespace-nowrap !text-[#111827] !text-sm"><strong className="!font-[800] whitespace-nowrap">{area.name}</strong></span><span className="min-w-0 whitespace-nowrap !text-[#4c5d7d] !text-sm !font-semibold">{area.hospital}</span><span className={`w-fit rounded-full px-3 py-1 !text-[0.76rem] !font-extrabold ${statusBadge(area.active)}`}>{area.active ? "Active" : "Deactivated"}</span><span className="justify-self-end whitespace-nowrap"><button className="flex items-center justify-center w-[88px] min-w-[88px] min-h-[42px] px-[18px] whitespace-nowrap rounded-lg bg-white border border-[#e2e8f0] !text-[#334155] !text-sm !font-bold hover:bg-[#f8fafc] transition-colors cursor-pointer" type="button" onClick={() => { setEditingDutyArea(area); setEditDutyHospitalId(String(area.hospitalId)); }}>Edit</button></span></div>)}
        ...
        <div className="flex items-center min-h-[48px] px-4 rounded-lg bg-[#e9f8ef] !text-[#078033] !text-sm !font-bold border border-[#bbf7d0]" role="status" aria-live="polite">{isLoading ? "Loading location data..." : message}</div>
      </main>
