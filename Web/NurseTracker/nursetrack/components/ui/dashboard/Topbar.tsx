...
export function Topbar({ titleKicker, title, onMenuClick, backHref }: TopbarProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-[50] flex items-center justify-between gap-5 min-h-[76px] px-[clamp(20px,4vw,34px)] py-[14px] border-b border-[#e5eaf1] bg-white/[0.94] backdrop-blur-[12px]">
        <div className="flex items-center gap-3 min-w-0">
          <button
            className="inline-flex lg:hidden items-center justify-center min-h-[38px] p-2 border border-[#dbe3ee] rounded-lg bg-white !text-[#344054] cursor-pointer hover:bg-[#f8fafc] hover:border-[#cbd5e1] hover:!text-[#0f172a] hover:shadow-sm hover:-translate-y-px transition-all"
            type="button"
            onClick={onMenuClick}
          ...
          />
          <div>
            <p className="m-0 mb-[2px] !text-[#64748b] !text-[0.78rem] !font-[800] uppercase tracking-wide">{titleKicker}</p>
            <h1 className="m-0 !text-[#111827] !text-[1.35rem] leading-[1.1] !font-[850]">{title}</h1>
          </div>
        </div>
