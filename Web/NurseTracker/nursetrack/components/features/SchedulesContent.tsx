            ...
            <>
              ...
              <div className="grid w-full min-w-0 grid-cols-7 gap-3 rounded-lg border border-[#e4e7ec]/92 bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(249,250,251,0.9)),#f8fafc] p-[14px] shadow-[inset_0_1px_0_rgba(255,255,255,0.9)] max-[640px]:gap-1.5 max-[640px]:p-2 max-[420px]:gap-1">
                ...
                {isLoading ? Array.from({ length: 42 }).map((_, index) => (
                  ...
                  const isMixedCanceledDay = daySchedules.length > 1 && activeDaySchedules.length > 0 && activeDaySchedules.length < daySchedules.length;
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex min-w-0 flex-col overflow-hidden rounded-lg border p-3 text-left outline-none min-h-[110px] max-[640px]:min-h-[72px] max-[640px]:p-1.5 max-[420px]:min-h-[60px] max-[420px]:rounded-md
                        ${sched ? isCanceledOnly ? "cursor-pointer border-[#fecaca] bg-[#fef2f2] shadow-[0_12px_30px_rgba(185,28,28,0.08)] transition-all hover:-translate-y-0.5 hover:border-[#fca5a5] hover:shadow-[0_16px_34px_rgba(185,28,28,0.1)] before:absolute before:inset-[0_auto_0_0] before:w-1 before:bg-[#ef4444]" : isMixedCanceledDay ? "cursor-pointer border-[#d89b16]/80 bg-[linear-gradient(145deg,#fff8d9_0%,#fff3bc_48%,#f7dfe2_100%)] shadow-[0_12px_30px_rgba(138,37,44,0.10)] transition-all hover:-translate-y-0.5 hover:border-[#8a252c]/45 hover:shadow-[0_16px_34px_rgba(138,37,44,0.13)] before:absolute before:inset-[0_auto_0_0] before:w-1 before:bg-[linear-gradient(180deg,#8A252C,#ffcf01)]" : "cursor-pointer border-[#ffcf01]/82 bg-[linear-gradient(145deg,#fff8d9_0%,#fff3bc_58%,#fffdf4_100%)] shadow-[0_12px_30px_rgba(161,92,7,0.1)] transition-all hover:-translate-y-0.5 hover:border-[#8a252c]/34 hover:shadow-[0_16px_34px_rgba(32,33,36,0.11)] before:absolute before:inset-[0_auto_0_0] before:w-1 before:bg-[#ffcf01]" : "cursor-default border-[#e4e7ec]/95 bg-[#fcfcfd] shadow-[0_1px_2px_rgba(32,33,36,0.03)]"}
                        ${isToday && !sched ? "!border-[#8a252c]/50 !bg-[linear-gradient(135deg,#fff8d6_0%,#fafafb_100%)] !shadow-[0_12px_26px_rgba(138,37,44,0.08)]" : ""}
                        ${isOtherMonth ? "opacity-[0.45] saturate-[0.55] hover:translate-y-0 hover:shadow-[0_1px_2px_rgba(32,33,36,0.03)]" : ""}
                      `}
                    >
                      <span className={`inline-flex w-fit items-center justify-center rounded-lg uppercase min-h-[28px] min-w-[28px] !text-[0.76rem] !font-[900] max-[640px]:min-h-[22px] max-[640px]:min-w-[22px] max-[640px]:!text-[0.66rem] max-[420px]:min-h-[20px] max-[420px]:min-w-[20px] max-[420px]:!text-[0.6rem]
                        ${isToday ? "!bg-[#8A252C] !text-white" : isOtherMonth ? "!bg-transparent !text-[#98a2b3]" : sched ? "!bg-[#8a252c]/10 !text-[#8a252c]" : "!text-[#475467]"}
                      `}>{cell.day}</span>
                       {sched && <strong className={`mt-[10px] block max-w-full truncate !text-[0.88rem] !font-[850] leading-[1.25] max-[640px]:mt-auto max-[640px]:rounded-full max-[640px]:bg-white/80 max-[640px]:px-1 max-[640px]:py-[2px] max-[640px]:text-center max-[640px]:!text-[0.52rem] max-[420px]:!text-[0.48rem] ${isOtherMonth ? "!text-[#98a2b3]" : "!text-[#111827]"}`}>{isCanceledOnly ? "Canceled" : daySchedules.length > 1 ? `${daySchedules.length}` : sched.area}</strong>}
                       {sched && <p className={`m-[6px_0_0] max-w-full truncate !text-[0.76rem] !font-[800] leading-[1.4] max-[640px]:hidden ${isOtherMonth ? "!text-[#98a2b3]" : "!text-[#344054]"}`}>{isCanceledOnly ? "No active schedule" : daySchedules.length > 1 ? "Multiple Duties" : sched.hospital}</p>}
