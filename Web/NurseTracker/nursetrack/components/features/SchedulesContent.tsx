                ...
                {isLoading ? Array.from({ length: 42 }).map((_, index) => (
                  ...
                  return (
                    ...
                    >
                      ...
                      `}>{cell.day}</span>
                       {sched && <strong className={`mt-[10px] block max-w-full truncate !text-[0.88rem] !font-[850] leading-[1.25] max-[640px]:mt-auto max-[640px]:rounded-full max-[640px]:bg-white/80 max-[640px]:px-1 max-[640px]:py-[2px] max-[640px]:text-center max-[640px]:!text-[0.52rem] max-[420px]:!text-[0.48rem] ${isOtherMonth ? "!text-[#98a2b3]" : "!text-[#111827]"}`}>{isCanceledOnly ? "Canceled" : activeDaySchedules.length > 1 ? `${activeDaySchedules.length}` : sched.area}</strong>}
                       {sched && <p className={`m-[6px_0_0] max-w-full truncate !text-[0.76rem] !font-[800] leading-[1.4] max-[640px]:hidden ${isOtherMonth ? "!text-[#98a2b3]" : "!text-[#344054]"}`}>{isCanceledOnly ? "No active schedule" : activeDaySchedules.length > 1 ? "Multiple Duties" : sched.hospital}</p>}
