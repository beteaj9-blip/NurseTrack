...
export function StudentProgressDetailContent({ basePath }: { basePath: string; searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const { data: allCases = [] } = useAllClinicalCases(isAllSection, isChair && user?.id != null ? String(user.id) : undefined);
  const cases = (isAllSection ? allCases : instructorCases).filter((record: any) => String(record.studentId) === String(studentId));
  const { data: instructorDutyRecords = [] } = useInstructorAttendance();
  const { data: allDutyRecords = [] } = useAllAttendance(isAllSection, isChair && user?.id != null ? String(user.id) : undefined);
  const dutyRecords = (isAllSection ? allDutyRecords : instructorDutyRecords).filter((record: any) => String(record.studentId) === String(studentId));
  const { data: instructorExtensionDays = [] } = useInstructorExtensionDays();
  const { data: allExtensionDays = [] } = useAllExtensionDays(undefined, isAllSection, isChair && user?.id != null ? String(user.id) : undefined);
  const extensionDays = (isAllSection ? allExtensionDays : instructorExtensionDays).filter((record: any) => String(record.studentId) === String(studentId));
  ...
  const requirements = buildRequirements(cases as any[]);
  ...
  return (
    <main className="p-[clamp(24px,4vw,42px)] min-h-[calc(100vh-64px)]">
      <section className="flex items-center justify-between gap-[28px] p-[clamp(24px,4vw,34px)] border border-[#e2e8f0] rounded-[8px] bg-white shadow-[0_16px_44px_rgba(32,33,36,0.07)] mb-[18px]">
        ...
        <span className="inline-flex items-center justify-start w-max max-w-full min-h-[28px] px-[10px] py-[6px] rounded-full bg-[#fff8e1] !text-[#6c4c00] !text-[0.76rem] !font-extrabold whitespace-nowrap">In progress</span>
      </section>

      <section className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-[18px] mb-[18px]" aria-label="Student progress summary">
        <SummaryCard icon="calendar" status="Open" title="Extension Days" description={`${activeExtensionDayTotal} extension days recorded`} />
        <SummaryCard icon="cases" status="In progress" title="Clinical Cases" description={`${completedCases} of ${totalRequiredCases} cases completed`} />
        <SummaryCard icon="alert" status="Open" title="Pending Items" description={`${pending} records need student or instructor action`} />
      </section>
...
function buildRequirements(records: any[]): RequirementGroup[] {
  const count = (label: string) => Math.min(records.filter((record) => record.status === "APPROVED" && record.category === label).length, 3);
  return [
    { code: "DR", label: "Delivery Room Cases", items: ["Handled Cases", "Assisted Cases", "Newborn Care", "Labor Watch"].map((label) => ({ label, completed: count(label), total: 3 })) },
    { code: "OR", label: "Operating Room Cases", items: ["Minor Cases", "Major Cases - Scrub", "Major Cases - Circulating"].map((label) => ({ label, completed: count(label), total: 3 })) },
  ];
}
