...
import { useCancelExtensionDay, useCreateExtensionDay, useInstructorExtensionDays, useUpdateExtensionDay } from "@/core/api/hooks/useExtensionDays";
...
export function ExtensionDaysDetailContent({ searchParams: searchParamsPromise }: { basePath: string; searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const { data: attendance = [], isLoading: attendanceLoading } = useInstructorAttendance(instructorId);
  const { data: extensionDays = [], isLoading: extensionLoading } = useInstructorExtensionDays(instructorId, studentId);
  ...
  const student = {
    name: firstRecord?.studentName || "Selected Student",
    profileImageUrl: firstRecord?.studentProfileImageUrl || "",
    id: firstRecord?.studentSchoolId || studentId || "",
    section: firstRecord?.studentSection || "Nursing Student",
  ...
  const isLoading = attendanceLoading || extensionLoading;
    ...
    <>
      <main className="p-[clamp(24px,4vw,42px)] min-h-[calc(100vh-64px)] grid gap-[1.25rem] content-start">
        ...
        <section className="border border-[#e2e8f0] rounded-xl bg-white shadow-[0_14px_34px_rgba(15,23,42,0.06)] p-[1.5rem]">
          ...
          {history.length > 0 ? <div className="grid gap-[0.85rem]">{history.map((record: any) => <div key={`${record.source}-${record.id}`} className="grid grid-cols-[3.75rem_minmax(210px,1fr)_minmax(190px,0.82fr)_minmax(240px,1.05fr)_minmax(190px,auto)] gap-[1rem] items-center p-[1rem_1.1rem] border border-[#dbe3ee] rounded-[0.85rem] bg-white max-[980px]:grid-cols-1 max-[980px]:justify-items-start" style={{ opacity: record.status === "CANCELED" ? 0.6 : 1 }}><ProfileAvatar name={student.name} imageUrl={student.profileImageUrl} size={48} /><div><strong className="block !text-[#111827] !text-[0.96rem] leading-[1.25]">{student.name}</strong><small className="block mt-[0.18rem] !text-[#475569] !text-[0.88rem] !font-[700] leading-[1.35]">{student.section} - {student.id}</small></div><div><strong className="block !text-[#111827] !text-[0.96rem] leading-[1.25]">{record.days} extension day{record.days > 1 ? "s" : ""}</strong><small className="block mt-[0.18rem] !text-[#475569] !text-[0.88rem] !font-[700] leading-[1.35]">{record.basis}</small></div><div><strong className="block !text-[#111827] !text-[0.96rem] leading-[1.25]">{record.reason}</strong><small className="block mt-[0.18rem] !text-[#475569] !text-[0.88rem] !font-[700] leading-[1.35]">Added {formatDate(record.createdAt)}</small></div><div className="flex items-center justify-end gap-[0.65rem] flex-wrap max-[980px]:justify-start"><span className={`inline-flex items-center w-max min-h-[28px] px-[10px] py-[6px] rounded-full !text-[0.76rem] !font-extrabold whitespace-nowrap ${record.status === "ACTIVE" ? "bg-[#e9f8ef] !text-[#03703c]" : "bg-[#f1f5f9] !text-[#64748b]"}`}>{record.status === "ACTIVE" ? "Added" : "Canceled"}</span>{record.status === "ACTIVE" && record.source === "extension" && <div className="inline-flex items-center gap-[0.45rem]"><button className="inline-flex items-center justify-center min-h-[2.25rem] px-[0.7rem] py-[0.45rem] rounded-[0.45rem] bg-white border border-[#e2e8f0] !text-[#344054] !text-[0.78rem] !font-[800] whitespace-nowrap hover:border-[rgba(138,37,44,0.32)] transition-all cursor-pointer" type="button" onClick={() => editRecord(record)}>Edit</button><button className="inline-flex items-center justify-center min-h-[2.25rem] px-[0.7rem] py-[0.45rem] rounded-[0.45rem] bg-white border border-[#e2e8f0] !text-[#344054] !text-[0.78rem] !font-[800] whitespace-nowrap hover:border-[rgba(138,37,44,0.32)] transition-all cursor-pointer" type="button" onClick={() => { setRecordToCancel(String(record.id)); setShowCancelModal(true); }}>Cancel</button></div>}</div></div>)}</div> : <p className="m-0 border border-dashed border-[#cbd5e1] rounded-[0.8rem] bg-[#f8fafc] p-[1.2rem] !text-[#64748b] !font-[700] text-center">No extension day records yet for this student.</p>}
        </section>
      </main>
