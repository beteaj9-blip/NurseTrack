...
export function CiRecommendationsDetailContent({ basePath }: { basePath: string; searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const isFinalStatus = appeal?.status === "ACCEPTED" || appeal?.status === "REJECTED";

  const submitStatus = async (status: "ACCEPTED" | "REJECTED") => {
    if (!appealId) return;
    try {
      ...
      <section className="bg-white rounded-xl shadow-[0_14px_34px_rgba(15,23,42,0.06)] border border-[#e2e8f0] p-[2rem] w-full mt-0">
        ...
        <div className="border border-[#e2e8f0] rounded-[10px] p-[1.5rem]">
          <div className="flex justify-between items-start mb-[1.25rem] gap-4">
            ...
            <span className={`${statusClass(appeal.status)} px-[14px] py-[4px] rounded-full !text-[0.8rem] !font-[800]`}>{appeal.status}</span>
          </div>
          ...
          <div className="flex flex-col gap-[0.6rem]">
            ...
            <div className="bg-[#f8fafc] p-[1.25rem] rounded-[8px] border-l-[4px] border-[#facc15]"><small className="block !text-[0.7rem] !font-[800] !text-[#64748b] uppercase mb-[6px] tracking-[0.05em]">CI Recommendation</small>{isFinalStatus ? <strong className="!text-[0.95rem] !text-[#334155] !font-[800]">{appeal.instructorRemarks || "No recommendation remarks provided."}</strong> : <textarea value={remarks} onChange={(event) => setRemarks(event.target.value)} rows={4} className="w-full p-3 border border-[#dbe3ee] rounded-lg text-[#111827] font-medium bg-white resize-y" placeholder={appeal.instructorRemarks || "Add recommendation remarks for this appeal"} />}</div>
          </div>
          {!isFinalStatus && <div className="flex justify-end gap-[1rem] mt-[1.5rem]">
            <button type="button" disabled={isSaving} onClick={() => submitStatus("REJECTED")} className="bg-white border border-[#e2e8f0] p-[0.75rem_1.75rem] rounded-[8px] !font-[800] !text-[#475569] !text-[0.95rem] cursor-pointer hover:border-[#cbd5e1] hover:!text-[#334155] transition-colors disabled:opacity-60 disabled:cursor-not-allowed">{isSaving ? "Saving..." : "Mark as Rejected"}</button>
