...
export function SectionImportContent() {
...
function StudentTable({ title, tone, students, section, empty, onRemove }: { title: string; tone: "success" | "danger"; students: SectionStudent[]; section: string; empty: string; onRemove: (student: SectionStudent) => void }) {
  ...
  return <article className="rounded-xl border border-[#e2e8f0] bg-white shadow-[0_16px_44px_rgba(32,33,36,0.07)] overflow-hidden">
    ...
    <div className="min-w-0">
      <div className="grid grid-cols-[56px_minmax(180px,1.4fr)_minmax(120px,0.8fr)_minmax(96px,0.7fr)_minmax(120px,0.8fr)_110px] items-center gap-4 border-b border-[#e2e8f0] bg-white px-4 py-3 !text-[#475569] !text-[0.72rem] !font-[900] uppercase max-[760px]:hidden">
        <span>No.</span><span>Student</span><span>School ID</span><span>Level</span><span>Section</span><span>Action</span>
      </div>
      {students.length === 0 ? <div className="p-5 !text-[#64748b] !font-[800]">{empty}</div> : pageStudents.map((student) => <div key={studentKey(student)} className="grid grid-cols-[56px_minmax(180px,1.4fr)_minmax(120px,0.8fr)_minmax(96px,0.7fr)_minmax(120px,0.8fr)_110px] items-center gap-4 border-b border-[#e2e8f0] px-4 py-4 last:border-0 max-[760px]:grid-cols-[40px_minmax(0,1fr)_auto] max-[760px]:gap-3 max-[760px]:py-3">
        <span className="!font-[900] !text-[#111827]">{student.studentNo || "-"}</span>
        <span className="min-w-0"><div className="flex min-w-0 items-center gap-3">{student.matched && <ProfileAvatar name={student.databaseName || student.name || "Student"} imageUrl={student.profileImageUrl} size={38} />}<strong className="block min-w-0 break-words !text-[#111827] !font-[900]">{student.databaseName || student.name || "Unnamed student"}</strong></div></span>
        <span className="break-words !text-[#334155] !font-[800] max-[760px]:col-start-2">{student.schoolId || "-"}</span>
        <span className="!text-[#334155] !font-[800] max-[760px]:col-start-2">{student.level ? `Level ${student.level}` : student.courseYear || "-"}</span>
        <span className="break-words !text-[#334155] !font-[800] max-[760px]:col-start-2">{section || "-"}</span>
        <span className="max-[760px]:col-start-3 max-[760px]:row-start-1 max-[760px]:justify-self-end"><button type="button" onClick={() => onRemove(student)} className="min-h-[38px] px-4 rounded-lg bg-white border border-[#fca5a5] !text-[#b91c1c] !font-[900] cursor-pointer hover:bg-[#fef2f2]">Remove</button></span>
      </div>)}
    </div>
