...
const students = [
  { id: "22-1845-103", name: "Treasure Abadinas", initials: "TA", section: "BSN 3A", status: "Approved", statusClass: "status-verified" },
  { id: "12-3456-789", name: "Maria Cruz", initials: "MC", section: "BSN 3A", status: "Submitted", statusClass: "status-pending" },
  { id: "23-1788-402", name: "Lichael Ursulo", initials: "LU", section: "BSN 3A", status: "Not submitted", statusClass: "status-rejected" },
  { id: "23-1023-441", name: "Nicole Dela Pena", initials: "ND", section: "BSN 3A", status: "Approved", statusClass: "status-verified" },
  { id: "21-7740-118", name: "Zander Aligato", initials: "ZA", section: "BSN 3B", status: "Submitted", statusClass: "status-pending" },
  { id: "23-1782-221", name: "Jay Tiongzon", initials: "JT", section: "BSN 3B", status: "Not submitted", statusClass: "status-rejected" },
  { id: "22-2451-667", name: "Hannah Bautista", initials: "HB", section: "BSN 3B", status: "Approved", statusClass: "status-verified" },
  { id: "22-8820-431", name: "Rafael Castillo", initials: "RC", section: "BSN 3B", status: "Submitted", statusClass: "status-pending" },
  { id: "23-5531-208", name: "Bea Montes", initials: "BM", section: "BSN 3B", status: "Not submitted", statusClass: "status-rejected" },
  { id: "22-6102-719", name: "Janine Aquino", initials: "JA", section: "BSN 3C", status: "Approved", statusClass: "status-verified" },
  { id: "23-4190-778", name: "Miguel Reyes", initials: "MR", section: "BSN 3C", status: "Submitted", statusClass: "status-pending" },
  { id: "22-7304-122", name: "Patricia Uy", initials: "PU", section: "BSN 3C", status: "Not submitted", statusClass: "status-rejected" },
];
export function ClearanceContent({ basePath }: { basePath: string }) {
  ...
  const filtered = students.filter(s => {
    ...
    return (!search || s.name.toLowerCase().includes(q) || s.id.toLowerCase().includes(q) || s.section.toLowerCase().includes(q) || s.status.toLowerCase().includes(q))
      ...
      && (statusFilter === "all" || s.status.toLowerCase().replace(" ", "-") === statusFilter);
  });
    ...
    <main className="p-[clamp(24px,4vw,42px)] min-h-[calc(100vh-64px)]">
      <section className="bg-white rounded-xl shadow-[0_14px_34px_rgba(15,23,42,0.06)] border border-[#e2e8f0] p-[1.6rem_1.75rem_1.75rem]">
        <div className="flex items-start justify-between gap-[22px] mb-[1.1rem] border-b border-[#e5eaf1] pb-[1.1rem] flex-wrap">
          <h2 className="m-0 !text-[#111827] !text-[1.15rem] !font-bold tracking-[-0.03em]">All-Section Clearance List</h2>
          <div className="flex items-center justify-end flex-wrap gap-[0.65rem]">
            <span className={`inline-flex items-center w-max min-h-[28px] px-[10px] py-[6px] rounded-full !text-[0.76rem] !font-extrabold whitespace-nowrap ${submissionsOpen ? "bg-[#e9f8ef] !text-[#03703c]" : "bg-[#fff8e1] !text-[#6c4c00]"}`}>{submissionsOpen ? "Submissions open" : "Submissions closed"}</span>
            <button className={ghostBtn} type="button" onClick={() => setSubmissionsOpen(o => !o)}>{submissionsOpen ? "Disable submissions" : "Enable submissions"}</button>
            <span className="inline-flex items-center w-max min-h-[28px] px-[10px] py-[6px] rounded-full !text-[0.76rem] !font-extrabold bg-[#e9f8ef] !text-[#03703c]">{filtered.length} visible</span>
          </div>
        </div>
        <div className="grid gap-[1rem] mb-[1rem] grid-cols-3 max-[980px]:grid-cols-1">
          ...
          <label className={labelCls} htmlFor="cl-section">Section<select className={`${inputCls} cursor-pointer`} id="cl-section" value={sectionFilter} onChange={e => { setSectionFilter(e.target.value); setCurrentPage(1); }}><option value="all">All sections</option><option value="BSN 3A">BSN 3A</option><option value="BSN 3B">BSN 3B</option><option value="BSN 3C">BSN 3C</option><option value="BSN 4A">BSN 4A</option></select></label>
          <label className={labelCls} htmlFor="cl-status">Clearance<select className={`${inputCls} cursor-pointer`} id="cl-status" value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setCurrentPage(1); }}><option value="all">All statuses</option><option value="not-submitted">Not submitted</option><option value="submitted">Submitted</option><option value="approved">Approved</option></select></label>
        </div>
        <div className={`flex flex-col border border-[#e2e8f0] overflow-hidden bg-white ${totalPages > 1 ? "rounded-t-lg" : "rounded-lg"}`}>
          {paged.map((s, i) => {
            ...
            return (
              <Link key={s.id} href={`${basePath}/clearance/detail?student=${s.name.toLowerCase().replace(/ /g, '-')}`} className="relative pl-[72px] flex items-center gap-[1.25rem] w-full p-[1rem_1.5rem] border-b border-[#e2e8f0] bg-white hover:bg-[#f8fafc] hover:translate-x-[2px] transition-all cursor-pointer no-underline text-inherit last:border-b-0" role="link" tabIndex={0}>
        ...
        {filtered.length === 0 && <div className="m-0 mt-[1rem] border border-dashed border-[#cbd5e1] rounded-lg bg-[#f8fafc] p-[1.25rem] !text-[#64748b] !font-[700] text-center">No students match the selected clearance filters.</div>}
      </section>
    </main>
  );
}
